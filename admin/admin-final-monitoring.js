// Enhanced Admin Dashboard with Performance Monitoring - FINAL WORKING VERSION
...
class PerformanceMonitor {
    ...
    generateSampleData() {
        ...
        // Generate sample database operations
        const dbOperations = ['create', 'read', 'update', 'delete'];
        for (let i = 0; i < 20; i++) {
            this.metrics.databaseOperations.push({
                operation: dbOperations[Math.floor(Math.random() * dbOperations.length)],
                duration: Math.random() * 50 + 10,
                timestamp: Date.now() - (20 - i) * 60000
            });
        }

        // Generate sample memory usage
        for (let i = 0; i < 30; i++) {
            this.metrics.memoryUsage.push({
                used: Math.random() * 50000000 + 30000000,
                total: 100000000,
                limit: 200000000,
        ...
        // Generate sample sync operations
        for (let i = 0; i < 15; i++) {
        ...
        console.log('Sample data generated:', this.metrics);
    }
    ...
    updateDashboard() {
        ...
        this.updateUserStatistics(performanceSection);
    }
    ...
    updatePerformanceMetrics(container) {
        ...
        const recentActivity = this.metrics.userActivity.filter(
        ...
        metricsContainer.innerHTML = `
            ...
            <div class="metric-card">
                <h4>用戶活動頻率</h4>
                <span class="metric-value">${recentActivity}/分鐘</span>
            </div>
        `;
    }
    ...
    updateUserStatistics(container) {
        const statsContainer = container.querySelector('.user-statistics');
        if (!statsContainer) return;

        // Load contact data for industry statistics
        this.loadIndustryStatistics(statsContainer);
    }

    async loadIndustryStatistics(container) {
        try {
            if (!window.databaseManager) {
                this.generateSampleIndustryData(container);
                return;
            }

            const contacts = await window.databaseManager.read('contacts');
            const industryStats = this.calculateIndustryStats(contacts);
            this.renderIndustryChart(container, industryStats);
        } catch (error) {
    ...
    calculateIndustryStats(contacts) {
        const industryMap = {
            '工業五金': 0,
            '機械零件': 0,
            '建材工程': 0,
        ...
        contacts.forEach(contact => {
            const service = contact.service || '其他';
            if (industryMap.hasOwnProperty(service)) {
                industryMap[service]++;
            } else {
                industryMap['其他']++;
            }
        ...
        return Object.entries(industryMap).map(([industry, count]) => ({
            industry,
            count
        }));
    }

    generateSampleIndustryData(container) {
        console.log('Generating sample industry data...');
        
        const sampleData = [
            { industry: '工業五金', count: 45 },
            { industry: '機械零件', count: 32 },
            { industry: '建材工程', count: 28 },
            { industry: '客製採購品', count: 18 },
            { industry: '其他', count: 12 }
        ];

        this.renderIndustryChart(container, sampleData);
    }

    renderIndustryChart(container, data) {
        let chartContainer = container.querySelector('.industry-chart');
        if (!chartContainer) {
            chartContainer = document.createElement('div');
            chartContainer.className = 'industry-chart';
            container.appendChild(chartContainer);
        }

        chartContainer.innerHTML = `
            <h4>用戶詢問產業類別統計</h4>
            <canvas id="industry-canvas" width="400" height="300"></canvas>
        `;

        setTimeout(() => {
            this.drawBarChart('industry-canvas', data);
        }, 100);
    }
    ...
    drawBarChart(canvasId, data) {
        ...
        data.forEach((item, index) => {
            ...
            ctx.textAlign = 'right';
            ctx.fillText(item.industry, 0, 0);
            ctx.restore();
        });
